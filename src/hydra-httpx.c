...
int32_t webport;
int32_t httpx_auth_mechanism = AUTH_UNASSIGNED;

int32_t start_httpx(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE *fp, char *type, ptr_header_node ptr_head) {
  char *empty = "";
  ...
  if (httpx_buf == NULL && httpx_auth_mechanism == AUTH_DIGESTMD5)
    httpx_auth_mechanism = AUTH_BASIC;

  if (use_proxy > 0 && proxy_count > 0)
    selected_proxy = random() % proxy_count;

  switch (httpx_auth_mechanism) {
  case AUTH_BASIC:
  ...
  case AUTH_DIGESTMD5: {
    ...
    pbuffer = hydra_strcasestr(httpx_buf, "WWW-Authenticate: Digest ");
  ...
  case AUTH_NTLM: {
    ...
    httpx_buf = hydra_receive_line(s);
    if (httpx_buf == NULL) {
      if (verbose)
    ...
    pos = hydra_strcasestr(httpx_buf, "WWW-Authenticate: NTLM ");
    if (pos != NULL) {
      char *str;
  ...
  while (httpx_buf != NULL && (strstr(httpx_buf, "HTTPx/1.") == NULL || (strchr(httpx_buf, '\n') == NULL && complete_line == 0))) {
    if (debug)
      printf("il: %d, tmpreplybuf: %s, httpx_buf: %s\n", complete_line, tmpreplybuf, httpx_buf);
    if (tmpreplybuf[0] == 0 && strstr(httpx_buf, "HTTPx/1.") != NULL) {
      strncpy(tmpreplybuf, httpx_buf, sizeof(tmpreplybuf) - 1);
      tmpreplybuf[sizeof(tmpreplybuf) - 1] = 0;
      free(httpx_buf);
      httpx_buf = hydra_receive_line(s);
    } else if (tmpreplybuf[0] != 0) {
      complete_line = 1;
      if ((tmpreplybufptr = malloc(strlen(tmpreplybuf) + strlen(httpx_buf) + 1)) != NULL) {
        strcpy(tmpreplybufptr, tmpreplybuf);
        strcat(tmpreplybufptr, httpx_buf);
        free(httpx_buf);
        httpx_buf = tmpreplybufptr;
        if (debug)
          printf("httpx_buf now: %s\n", httpx_buf);
      }
    } else {
      free(httpx_buf);
      httpx_buf = hydra_receive_line(s);
  ...
  ptr = ((char *)strchr(httpx_buf, ' '));
  if (ptr != NULL)
    ptr++;
  if (ptr != NULL && (*ptr == '2' || *ptr == '3' || strncmp(ptr, "403", 3) == 0 || strncmp(ptr, "404", 3) == 0)) {
#ifdef HAVE_PCRE
    if (end_condition_type >= 0 && hydra_string_match(httpx_buf, end_condition) != end_condition_type) {
#else
    if (end_condition_type >= 0 && (strstr(httpx_buf, end_condition) == NULL ? 0 : 1) != end_condition_type) {
#endif
      if (debug)
    ...
    if ((hydra_strcasestr(httpx_buf, "WWW-Authenticate: Basic") == NULL) && (httpx_auth_mechanism == AUTH_BASIC)) {
      // seems the auth supported is not Basic scheme so testing further
      int32_t find_auth = 0;

      if (hydra_strcasestr(httpx_buf, "WWW-Authenticate: NTLM") != NULL) {
        httpx_auth_mechanism = AUTH_NTLM;
        find_auth = 1;
      }
#ifdef LIBOPENSSL
      if (hydra_strcasestr(httpx_buf, "WWW-Authenticate: Digest") != NULL) {
        httpx_auth_mechanism = AUTH_DIGESTMD5;
        find_auth = 1;
      }
#endif
...
void service_httpx(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname, char *type) {
  int32_t run = 1, next_run = 1, sock = -1;
  int32_t myport = PORT_HTTPx, mysslport = PORT_HTTPx_SSL;
  char *ptr, *ptr2;
  ...
  if (!parse_options(optional1,
                     &ptr_head)) // this function is in hydra-httpx-form.c !!
    run = 4;

  if (httpx_auth_mechanism == AUTH_UNASSIGNED)
    httpx_auth_mechanism = AUTH_BASIC;
  ...
  while (1) {
    ...
    case 2: /* run the cracking function */
      next_run = start_httpx(sock, ip, port, options, miscptr, fp, type, ptr_head);
      break;
    case 3: /* clean exit */
      if (sock >= 0)
        sock = hydra_disconnect(sock);
...
void service_httpx_get(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) { service_httpx(ip, sp, options, miscptr, fp, port, hostname, "GET"); }

void service_httpx_post(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) { service_httpx(ip, sp, options, miscptr, fp, port, hostname, "POST"); }

void service_httpx_head(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) { service_httpx(ip, sp, options, miscptr, fp, port, hostname, "HEAD"); }

int32_t service_httpx_init(char *ip, int32_t sp, unsigned char options, char *miscptr, FILE *fp, int32_t port, char *hostname) {
...
void usage_httpx(const char *service) {
  printf("Module %s requires the page to authenticate.\n"
         "The following parameters are optional:\n"
         " (a|A)=auth-type   specify authentication mechanism to use: BASIC, "
         "NTLM or MD5\n"
         " (h|H)=My-Hdr\\: foo   to send a user defined HTTPx header with each "
         "request\n"
         " (F|S)=check for text in the HTTPx reply. S= means if this text is "
         "found, a\n"
         "       valid account has been found, F= means if this string is "
         "present the\n"
         "       combination is invalid. Note: this must be the last option "
         "supplied.\n"
         "For example:  \"/secret\" or \"httpx://bla.com/foo/bar:H=Cookie\\: "
         "sessid=aaaa\" or \"httpxs://test.com:8080/members:A=NTLM\"\n\n",
         service);
}
